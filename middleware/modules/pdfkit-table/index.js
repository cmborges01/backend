  ...
  table(table, options, callback) {
    return new Promise((resolve, reject) => {
      try {
        ...
        let mergeRow = options.mergeRow || false;
        let muteText = options.muteText || false;
        let hideOutterGrid = options.hideOutterGrid || false;
        let fontFamilyHeader = options.fontFamilyHeader
          ? config.fonts + options.fontFamilyHeader + ".ttf"
          : "Helvetica-Bold" || "Helvetica-Bold";
        let fontColorHeader = options.fontColorHeader || "black";
        let fontSizeHeader = options.fontSizeHeader || 8;
        let fontSizeRows = options.fontSizeRows || 8;
        let fontFamilyRows = options.fontFamilyRows
          ? config.fonts + options.fontFamilyRows + ".ttf"
          : "Helvetica" || "Helvetica";
        let fontColorRows = options.fontColorRows || "black";
        const prepareHeader =
          options.prepareHeader ||
          (() =>
            this.fillColor(fontColorHeader)
              .font(fontFamilyHeader)
              .fontSize(8)
              .fill());
        const prepareRow =
          options.prepareRow ||
          ((row, indexColumn, indexRow, rectRow, rectCell) =>
            this.fillColor(fontColorRows)
              .font(fontFamilyRows)
              .fontSize(8)
              .fill());
          ...
          if (!options.hideHeader && table.headers.length > 0) {
            ...
            if (typeof table.headers[0] === "string") {
              ...
              table.headers.forEach((dataHeader, i) => {
                ...
                align = headerAlign || align || "center";
                // force number
                width = width >> 0;
                ...
                if (i === 0) {
                  this.lineWidth(0.5)
                    .moveTo(
                      lastPositionX,
                      startY - columnSpacing - rowDistance * 2
                    )
                    .lineTo(
                      lastPositionX,
                      startY -
                  ...
                  this.lineWidth(0.5)
                    ...
                    .lineTo(
                      lastPositionX + width * table.headers.length,
                      startY - columnSpacing - rowDistance * 2
                    )
                    .stroke();
                }
                ...
                if (!mergeRow) {
                  ...
                  if (i % 2 === 0 && index === 0)
                    this.text(
                      cell,
                      lastPositionX + cellPadding.left,
                      startY + rowHeight / 2 + topTextToAlignVertically,
                      {
                        width:
                          columnSizes[index] -
                          (cellPadding.left + cellPadding.right),
